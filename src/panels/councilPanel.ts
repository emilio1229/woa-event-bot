...
export function buildCouncilPanel() {
  const embed = new EmbedBuilder()
    ...
    .setDescription("Welcome, Council.\n\nThe command center of The Wizards of Ark. Manage the Realm without memorizing commands.")
    .addFields(
      { name: "💎 Economy", value: "Assign/remove Sigils", inline: true },
      { name: "🎟️ Raffles", value: "Start or end community giveaways", inline: true },
      { name: "🏆 Events", value: "Create scheduled gatherings", inline: true },
      { name: "📜 Bounties", value: "Create the weekly Hunt", inline: true },
      { name: "🎁 Rewards", value: "View the Sigil shop", inline: true },
...
export async function handleCouncilPanel(interaction: Interaction) {
  ...
  if (interaction.isStringSelectMenu()) {
    await handleCouncilStringSelect(interaction);
    return true;
  }
...
async function handleCouncilStringSelect(interaction: StringSelectMenuInteraction) {
  if (interaction.customId === `${COUNCIL_PREFIX}:raffle:end:select`) {
    await endRaffleForCouncil(interaction, interaction.values[0]);
    return;
  }
  if (interaction.customId.startsWith(`${COUNCIL_PREFIX}:bounty:stat:`)) {
    const index = Number(interaction.customId.split(":").at(-1));
    const draft = bountyDrafts.get(interaction.user.id);
    if (!draft || !Number.isInteger(index) || index < 0 || index > 3) {
      await interaction.reply({ content: "❌ That bounty draft has expired. Start the bounty again.", ephemeral: true });
      return;
    }
    draft.stats[index] = interaction.values[0];
...
async function handleCouncilModal(interaction: ModalSubmitInteraction) {
  ...
  if (id.startsWith(`${COUNCIL_PREFIX}:event:modal:`)) {
    ...
    const roleId = id.split(":").at(-1) === "none" ? null : id.split(":").at(-1) || null;
    const timezone = getTimezoneForLocale(interaction.locale ?? "en-US", interaction.user.id);
    const millis = parseTime(time, timezone);
    if (!title || !description || millis === null || Number.isNaN(millis) || millis <= Date.now()) {
      await interaction.reply({ content: "❌ Please provide a title, description, and valid future event time.", ephemeral: true });
      return;
    }
  ...
  if (id.startsWith(`${COUNCIL_PREFIX}:bounty:modal:`)) {
    ...
    bountyDrafts.set(interaction.user.id, { guildId: guild.id, channelId: channel.id, roleId, dinos, bonus, stats: ["Melee", "Melee", "Melee", "Melee"] });
    await showBountyStatPicker(interaction, interaction.user.id);
  }
}
...
function buildEventModal(roleId: string | null) {
  ...
  modal.addComponents(
    ...
    inputRow("description", "Description", "What is happening? Include the important details.", TextInputStyle.Paragraph),
    inputRow("notes", "Notes", "Optional extra notes for players", TextInputStyle.Paragraph, false)
  );
  return modal;
}
...
async function showBountyStatPicker(interaction: ModalSubmitInteraction, userId: string) {
  const draft = bountyDrafts.get(userId);
  if (!draft) return;
  const rows: ActionRowBuilder<StringSelectMenuBuilder>[] = [];
  for (let index = 0; index < 4; index += 1) {
    const menu = new StringSelectMenuBuilder()
      .setCustomId(`${COUNCIL_PREFIX}:bounty:stat:${index}`)
      .setPlaceholder(`${draft.dinos[index]} — choose target stat`)
      .setMinValues(1)
      .setMaxValues(1)
      .addOptions(BOUNTY_STATS.map(stat => ({ label: stat, value: stat, default: draft.stats[index] === stat })));
    rows.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu));
  }
  rows.push(new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(
    new StringSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:bounty:stat:summary`).setPlaceholder("Review target stats").setMinValues(1).setMaxValues(1).addOptions({ label: "Review the selections", value: "review" })
  ));
  const buttons = new ActionRowBuilder<ButtonBuilder>().addComponents(
    button("🎲 Randomize All & Post", `${COUNCIL_PREFIX}:bounty:randomize`, ButtonStyle.Secondary),
    button("📜 Post Bounty", `${COUNCIL_PREFIX}:bounty:post`, ButtonStyle.Success)
  );
  await interaction.reply({ content: "🜁 **The Hunt must be inscribed.** Choose the stat for each creature below. The locked stat range is **40–50**. You may also randomize all four targets.", components: [...rows.slice(0, 4), buttons], ephemeral: true });
}
...
async function postBounty(interaction: ButtonInteraction, userId: string) {
  ...
  const targets = draft.dinos.map((dino, index) => `• **${dino}** — **${draft.stats[index]}** ▸ 40–50`).join("\n");
  const ritual = [
    "**🜁 THE RITUAL OF THE HUNT**",
    "",
    "Every offering must satisfy the ancient rules of the Hunt:",
    "• Creature must spawn at the **server’s max wild level**",
    "• **Perfect taming effectiveness** is required",
    "• Final tame level must fall within the **perfect-tame range (202–224)**",
    "• Screenshot the stat page showing the final level",
    "• Cryopod the creature before submission",
    "• Open a **#🎫➖bounty-turn-in-ticket** and present your offering to an admin",
    "",
    "**⚔️ TARGETS OF THE WEEK**",
    targets,
    "",
    draft.bonus ? `⚡ **Bonus Bounty:** ${draft.bonus}` : "",
    "",
    `🜁 **Summoned Order:** <@&${draft.roleId}>`,
    "",
    "The stat target is **40–50** for every offering. Read the ritual carefully before you begin your hunt."
  ].filter(Boolean).join("\n");
  ...
  if (message) await bountyStore.setMessageId(record.id, message.id);
...
async function randomizeAndPostBounty(interaction: ButtonInteraction, userId: string) {
  const draft = bountyDrafts.get(userId);
  if (!draft) {
    await interaction.reply({ content: "❌ That bounty draft has expired. Start the bounty again.", ephemeral: true });
    return;
  }
  draft.stats = draft.dinos.map(() => BOUNTY_STATS[Math.floor(Math.random() * BOUNTY_STATS.length)]);
  await postBounty(interaction, userId);
}
...
async function openSigilAdjustment(interaction: UserSelectMenuInteraction) {
  ...
  modal.addComponents(inputRow("amount", "Sigil amount", "25 to award, -25 to remove", TextInputStyle.Short), inputRow("reason", "Reason", "Event reward, correction, etc.", TextInputStyle.Paragraph));
...
async function startRaffleModal(interaction: ButtonInteraction) {
  const roleMenu = new RoleSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:raffle:role`).setPlaceholder("Choose the Discord role to notify for this giveaway.").setMinValues(1).setMaxValues(1);
...
async function startRaffleEndPicker(interaction: ButtonInteraction) {
  const raffles = (await raffleStore.all()).filter(raffle => raffle.guildId === interaction.guildId && !raffle.ended && raffle.endsAt > Date.now()).slice(0, 25);
  if (raffles.length === 0) {
    await interaction.reply({ content: "❌ There are no active giveaways to end.", ephemeral: true });
    return;
  }
  const menu = new StringSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:raffle:end:select`).setPlaceholder("Choose the active giveaway to end");
  menu.addOptions(raffles.map(raffle => ({ label: (raffle.name || "WoA Community Giveaway").slice(0, 100), value: raffle.id, description: `Prize: ${raffle.prize}`.slice(0, 100) })));
  await interaction.reply({ content: "🔮 **End a Giveaway**\nChoose the active ritual you want to conclude. The winner will be chosen from the entries.", components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)], ephemeral: true });
}
...
async function endRaffleForCouncil(interaction: StringSelectMenuInteraction, raffleId: string) {
  ...
  if (!raffle || raffle.guildId !== interaction.guildId || raffle.ended || raffle.endsAt <= Date.now()) {
    await interaction.update({ content: "❌ That giveaway is no longer active.", components: [] });
    return;
  }
  const entries = raffle.entries ?? [];
  const winnerId = raffle.winnerId ?? (entries.length ? entries[Math.floor(Math.random() * entries.length)] : null);
  if (winnerId) {
  ...
  try {
    ...
    if (!announcementChannel || !("send" in announcementChannel)) throw new Error("Raffle announcement channel is unavailable.");
    const grandEmbed = new EmbedBuilder()
      .setColor(0xFF4500)
      .setTitle("✨ A Champion Has Been Chosen ✨")
      .setDescription(winnerId ? "The sigils have chosen their champion." : "The ritual concludes with no champion.")
      .addFields(
        { name: "👑 Winner", value: winnerId ? `<@${winnerId}>` : "No entries", inline: false },
    ...
    await announcementChannel.send({ embeds: [grandEmbed], files: [attachment], allowedMentions: { users: winnerId ? [winnerId] : [] } });
    ...
    if (messageChannel && "messages" in messageChannel && raffle.messageId) {
    ...
    await interaction.update({ content: winnerId ? `🔮 Giveaway ended. Winner: <@${winnerId}>` : "🔮 Giveaway ended with no entries.", components: [] });
  } catch (error) {
    console.error("Council raffle end failed:", error);
    await interaction.update({ content: "❌ The giveaway could not be safely concluded. It remains active for a retry.", components: [] });
  }
}

async function startBountyModal(interaction: ButtonInteraction) {
  const roleMenu = new RoleSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:bounty:role`).setPlaceholder("Choose the role to notify for this Hunt").setMinValues(1).setMaxValues(1);
...
async function showRaffles(interaction: ButtonInteraction) {
  const raffles = (await raffleStore.all()).filter(raffle => raffle.guildId === interaction.guildId && !raffle.ended && raffle.endsAt > Date.now());
  const description = raffles.length
    ? raffles.slice(0, 10).map(raffle => `🎟️ **${raffle.name}** — ${raffle.prize}\nEnds <t:${Math.floor(raffle.endsAt / 1000)}:R> • **${raffle.entries.length} entries**`).join("\n\n")
    : "No active community giveaways.";
  await interaction.update({ embeds: [new EmbedBuilder().setTitle("🎟️ Raffles").setDescription(description)], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(button("✨ Start Giveaway", `${COUNCIL_PREFIX}:raffles:start`), button("🛑 End Giveaway", `${COUNCIL_PREFIX}:raffles:end`, ButtonStyle.Danger)), backButtonRow()] });
}
...
async function showEvents(interaction: ButtonInteraction) {
  ...
  await interaction.update({ embeds: [new EmbedBuilder().setTitle("🏆 Events").setDescription(description.slice(0, 4000))], components: [new ActionRowBuilder<ButtonBuilder>().addComponents(button("✨ Start Event", `${COUNCIL_PREFIX}:events:start`)), backButtonRow()] });
}

async function showBounties(interaction: ButtonInteraction) {
  const bounties = await bountyStore.getActive(interaction.guildId ?? "");
  const description = bounties.length ? bounties.map(bounty => `📜 **${bounty.dinos.join(", ")}** — ${bounty.stats.join(", ")} ▸ 40–50`).join("\n") : "No active bounties.";
...
async function showRewards(interaction: ButtonInteraction) {
  const activeRaffles = (await raffleStore.all()).filter(raffle => raffle.guildId === interaction.guildId && Date.now() < raffle.endsAt && !raffle.ended);
  ...
  components.push(backButtonRow() as never);
  ...
  const embed = new EmbedBuilder().setTitle("🛍️ Sigil Shop").setDescription(["Trade your Sigils for community giveaway entries.", `Exchange rate: **${SIGILS_PER_RAFFLE_ENTRY} sigils = 1 raffle entry**.`, `Current balance: **${user.balance} sigils**.`].join("\n")).addFields({ name: "🎟️ Active Giveaways", value: activeText, inline: false }).setTimestamp();
  await interaction.update({ embeds: [embed], components });
}

async function showStatistics(interaction: ButtonInteraction) {
  const users = await discordDirectoryService.listMembers(interaction.guildId ?? "");
  const activeRaffles = (await raffleStore.all()).filter(raffle => raffle.guildId === interaction.guildId && !raffle.ended && raffle.endsAt > Date.now()).length;

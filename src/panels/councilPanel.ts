...
async function showRewards(interaction: ButtonInteraction) {
  ...
  const activeRaffleText = raffle ? `🎟️ **${raffle.name}** — ${raffle.prize}\nID: \`${raffle.id}\` • Ends <t:${Math.floor(raffle.endsAt / 1000)}:R>` : "No active community giveaway is available for redemption right now.";
  const embed = new EmbedBuilder().setTitle("🛍️ Sigil Shop").setDescription(["Trade your Sigils for community giveaway entries.", `Exchange rate: **${SIGILS_PER_RAFFLE_ENTRY} sigils = 1 raffle entry**.`, `Current balance: **${user.balance} sigils**.`].join("\n")).addFields({ name: "🎟️ Active Giveaway", value: activeRaffleText, inline: false }).setTimestamp();
...
async function showMembers(interaction: ButtonInteraction) {
  ...
  const description = council.length ? council.slice(0, 25).map(member => `🏛️ <@${member.discordUserId}>`).join("\n") : "No Council members found in the configured roles.";

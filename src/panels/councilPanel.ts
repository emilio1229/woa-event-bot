...
async function handleCouncilStringSelect(interaction: StringSelectMenuInteraction) {
  ...
  if (interaction.customId.startsWith(`${COUNCIL_PREFIX}:bounty:stat:`)) {
    ...
    if (!draft || !Number.isInteger(index) || index < 0 || index > 3) { await interaction.reply({ content: "❌ That bounty draft has expired. Start the bounty again.", flags: MessageFlags.Ephemeral }); return; }
...
async function handleCouncilModal(interaction: ModalSubmitInteraction) {
  ...
  if (id.startsWith(`${COUNCIL_PREFIX}:sigil:modal:`)) {
    ...
    if (!Number.isInteger(amount) || amount === 0 || !reason) { await interaction.reply({ content: "❌ Enter a non-zero whole-number amount and a reason.", flags: MessageFlags.Ephemeral }); return; }
    try { const updated = await sigilStore.award(guild.id, userId, amount, reason, interaction.user.id); const target = await guild.members.fetch(userId).catch(() => null); const embed = buildBalanceEmbed(target?.user ?? interaction.user, updated.balance, updated.transactions.slice(0, 5), amount > 0 ? "✨ Sigils Awarded" : "🜂 Sigils Removed", `${target ? target.toString() : `<@${userId}>`} has been ${amount > 0 ? "granted" : "charged"} **${Math.abs(amount)}** sigils.`); await interaction.reply({ embeds: [embed], flags: MessageFlags.Ephemeral }); }
    catch (error) { await interaction.reply({ content: `❌ ${error instanceof Error ? error.message : "Unable to update the sigil ledger."}`, flags: MessageFlags.Ephemeral }); }
    return;
  }
  if (id.startsWith(`${COUNCIL_PREFIX}:event:modal:`)) {
    const channel = interaction.channel; if (!channel || !channel.isSendable()) { await interaction.reply({ content: "❌ This control must be used in a channel where the bot can send messages.", flags: MessageFlags.Ephemeral }); return; }
    ...
    if (!title || !description || millis === null || Number.isNaN(millis) || millis <= Date.now()) { await interaction.reply({ content: "❌ Please provide a title, description, and valid future event time.", flags: MessageFlags.Ephemeral }); return; }
  ...
  if (id.startsWith(`${COUNCIL_PREFIX}:raffle:modal:`)) {
    ...
    if (!prize || endsAt === null || Number.isNaN(endsAt) || endsAt <= Date.now() || !/^\d{17,20}$/.test(roleId)) { await interaction.reply({ content: "❌ Provide a prize and a valid future end time.", flags: MessageFlags.Ephemeral }); return; }
    ...
    await raffleStore.setMessageId(raffle.id, message.id); await interaction.reply({ content: thread ? `✅ Giveaway started in <#${thread.id}>.` : "✅ Giveaway started.", flags: MessageFlags.Ephemeral }); return;
  }
  if (id.startsWith(`${COUNCIL_PREFIX}:bounty:modal:`)) {
    ...
    if (dinos.length !== 4 || dinos.some(dino => dino.length < 1) || !/^\d{17,20}$/.test(roleId)) { await interaction.reply({ content: "❌ Enter exactly four dino names separated by commas and select a valid Discord role.", flags: MessageFlags.Ephemeral }); return; }
...
async function showBountyStatPicker(interaction: ModalSubmitInteraction, userId: string) {
  ...
  await interaction.reply({ content: "🜁 **The Hunt must be inscribed.** Choose the stat for each creature below. The locked stat range is **40–50**. You may also randomize all four targets.", components: [...rows, buttons], flags: MessageFlags.Ephemeral });
}

async function postBounty(interaction: ButtonInteraction, userId: string) {
  const draft = bountyDrafts.get(userId); if (!draft) { await interaction.reply({ content: "❌ That bounty draft has expired. Start the bounty again.", flags: MessageFlags.Ephemeral }); return; }
  if (!interaction.channel?.isSendable()) { await interaction.reply({ content: "❌ This channel cannot receive the bounty announcement.", flags: MessageFlags.Ephemeral }); return; }
...
async function randomizeAndPostBounty(interaction: ButtonInteraction, userId: string) { const draft = bountyDrafts.get(userId); if (!draft) { await interaction.reply({ content: "❌ That bounty draft has expired. Start the bounty again.", flags: MessageFlags.Ephemeral }); return; } draft.stats = draft.dinos.map(() => BOUNTY_STATS[Math.floor(Math.random() * BOUNTY_STATS.length)]); await postBounty(interaction, userId); }
async function startSigilAssignment(interaction: ButtonInteraction) { const menu = new UserSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:sigil:user`).setPlaceholder("Select a player").setMinValues(1).setMaxValues(1); await interaction.reply({ content: "💎 Select the player whose Sigils you want to adjust.", components: [new ActionRowBuilder<UserSelectMenuBuilder>().addComponents(menu)], flags: MessageFlags.Ephemeral }); }
...
async function startEventModal(interaction: ButtonInteraction, roleId: string | null | undefined = undefined) { if (roleId !== undefined) { await interaction.showModal(buildEventModal(roleId)); return; } const roleMenu = new RoleSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:event:role`).setPlaceholder("Choose the role to notify"); const skip = button("No role to tag", `${COUNCIL_PREFIX}:events:no-role`, ButtonStyle.Secondary); await interaction.reply({ content: "🔔 Choose the Discord role to notify. You can also skip the role tag.", components: [new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(roleMenu), new ActionRowBuilder<ButtonBuilder>().addComponents(skip)], flags: MessageFlags.Ephemeral }); }
async function startRaffleModal(interaction: ButtonInteraction) { const roleMenu = new RoleSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:raffle:role`).setPlaceholder("Choose the Discord role to notify for this giveaway.").setMinValues(1).setMaxValues(1); await interaction.reply({ content: "🔔 Choose the Discord role to notify for this giveaway.", components: [new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(roleMenu)], flags: MessageFlags.Ephemeral }); }
async function startRaffleEndPicker(interaction: ButtonInteraction) { const raffles = (await raffleStore.all()).filter(raffle => raffle.guildId === interaction.guildId && !raffle.ended && raffle.endsAt > Date.now()).slice(0, 25); if (!raffles.length) { await interaction.reply({ content: "❌ There are no active giveaways to end.", flags: MessageFlags.Ephemeral }); return; } const menu = new StringSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:raffle:end:select`).setPlaceholder("Choose the active giveaway to end"); menu.addOptions(raffles.map(raffle => ({ label: (raffle.name || "WoA Community Giveaway").slice(0, 100), value: raffle.id, description: `Prize: ${raffle.prize}`.slice(0, 100) }))); await interaction.reply({ content: "🔮 **End a Giveaway**\nChoose the active ritual you want to conclude. The winner will be chosen from the entries.", components: [new ActionRowBuilder<StringSelectMenuBuilder>().addComponents(menu)], flags: MessageFlags.Ephemeral }); }
...
async function startBountyModal(interaction: ButtonInteraction) { const roleMenu = new RoleSelectMenuBuilder().setCustomId(`${COUNCIL_PREFIX}:bounty:role`).setPlaceholder("Choose the role to notify for this Hunt").setMinValues(1).setMaxValues(1); await interaction.reply({ content: "🔔 Choose the Discord role to notify for this bounty.", components: [new ActionRowBuilder<RoleSelectMenuBuilder>().addComponents(roleMenu)], flags: MessageFlags.Ephemeral }); }

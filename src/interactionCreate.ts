...
async function handleStatusSelection(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  if (!raffle) { try { await interaction.editReply({ content: "Selected ritual not found.", components: [] }); } catch {} return; }
  const embed = new EmbedBuilder().setTitle("🔮 Active Ritual Status").addFields(
...
async function handleEndSelection(interaction: StringSelectMenuInteraction): Promise<void> {
  ...
  try {
    ...
    if (winnerId) {
      ...
      if (isMessageCapableChannel(channel)) {
        const grandEmbed = new EmbedBuilder().setColor(0xFF4500).setTitle("✨ A Champion Has Been Chosen ✨").setDescription("The sigils have chosen their champion.").addFields(
        ...
        ).setFooter({ text: "Wizards of Ark • Ascension Complete" }).setTimestamp();
  ...
  } catch (error) { console.error("select_end_raffle error:", error); try { await interaction.editReply({ content: "An error occurred while ending the ritual.", components: [] }); } catch {} }
}
...
export async function handleInteraction(interaction: Interaction): Promise<void> {
  try {
    ...
    if (interaction.isButton()) {
      ...
      if ((interaction.customId === "bindSoul" || interaction.customId.startsWith("bindSoul_")) && raffleId && raffle) { await handleBindSoul(interaction, raffleId); return; }
      if ((interaction.customId === "unbindSoul" || interaction.customId.startsWith("unbindSoul_")) && raffleId && raffle) { await handleUnbindSoul(interaction, raffleId); return; }
      if (interaction.customId === "bindSoul" || interaction.customId === "unbindSoul") await interaction.reply({ content: "❌ This ritual is no longer active.", flags: MessageFlags.Ephemeral });
      return;
    }
    if (interaction.isModalSubmit() && interaction.customId.startsWith("sigil_redeem_modal:")) {
      if (!interaction.inGuild() || !interaction.guild) { await interaction.reply({ content: "❌ Sigil redemption can only be used inside a server raffle channel.", flags: MessageFlags.Ephemeral }); return; }
